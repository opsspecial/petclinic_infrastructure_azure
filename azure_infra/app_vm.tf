...
// //   template = file("${path.module}/Templates/cloudinit-app.tpl")
// }
...
resource "azurerm_virtual_machine" "app" {
  ...
  storage_os_disk {
    name              = "${var.prefix}-app-${count.index}-app-osdisk"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "${var.prefix}-app-${count.index}"
    admin_username = var.username
    // custom_data    = data.template_file.app.rendered
  }
